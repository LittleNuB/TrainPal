...
选中一条本地健身视频以后，TrainPal 不会立刻给出一份看起来完整的训练计划。它先检查整段来源，整理动作与对应时间，再把有疑问的地方留给人确认。用户可以改动作、顺序和参数，随后带着同一位小猫教练完成训练。
...
TrainPal 当前是竞赛与作品集原型。公开仓库包含完整 Web 应用、分析服务、训练流程与验证合同，运行真实云端分析仍需要自行配置 Provider。服务不可用时，产品会保留浏览与训练路径，也会如实说明分析没有完成。
...
- 受控演示来源可以帮助访客快速走通真实分析，它们来自明确清单，也不会回放缓存结果冒充新运行。
